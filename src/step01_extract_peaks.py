...
def compute_spike_magnitudes_and_times(sp, timestamps):
	"""
	Returns spike magnitudes and spike times.

	:param sp: spike matrix, like from sp.npy from suite2p folder, or step00_process_suite2p/Sp.npy
	:param timestamps: 1d np.array of timestamps
	:return:
		  sp_amp - list[np.array] spike amplitudes
		sp_times - list[np.array] spike times
	"""
	if sp.shape[-1] != timestamps.size:
		raise (ValueError("sp and timestamps are not compatible sizes. "))

	if sp.ndim == 1:
		sp_idx = np.argwhere(sp)
		sp_amp = sp[sp != 0]
		sp_times = timestamps[sp != 0]

	return sp_amp, sp_times


def compute_interval_spike_peaks(cells_x_time, timestamps, stim_ict, peak_start=0.5, peak_len=5.0, baseline_len=10.0):
	"""
	Computes spike peaks in time intervals after stimulus onset times.

	:param cells_x_time: spike matrix, like from step00_process_suite2p/Sp.npy
	:param timestamps: 1d np.array of timestamps
	:param stim_ict: 1d np.array of stimulus onset times
	:param peak_start: length of time after stim_ict to start calculating peak_mean
	:param peak_len: Time period (after peak_start) over which to calculate peak_mean
			         peak_mean = mean(dfof, peak_start: peak_start + peak_len)
	:param baseline_len: Length of time pre stim_ict over which to calculate baseline_mean and baseline_std
	:return: np.ndarray, [cells x trials]


	"""


def compute_interval_response_vectors(dfof, timestamps, stim_ict, peak_start=0.5, peak_len=5.0, baseline_len=10.0):
	"""
...
def extract(f):
	"""
    Load deconvolved calcium responses from suite2p/**/step00_process_suite2p (folder in the same directory as stat.npy)

    :param f: Path to stat.npy, uses this to parse fly information and load other dataset info
    :return: nt_metadata,  title_str,  C_dec
    """
...
def transform(dfof, timestamps, stim_ict, peak_start=0.5, peak_len=5, baseline_len=10):
	...
	:param peak_len:
	:param baseline_len:
	:return: dict peak_data (peak_mean,  peak_start,
				- peak_mean
				- peak_start
	"""
...
def main(f, respvec_ops=None):
	"""
	Converts contents of step00_process_suite2p (C_dec) to a peak data matrix [n_cells x trials]
